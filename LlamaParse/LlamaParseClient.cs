...
public partial class LlamaParseClient
{
    ...
    public IAsyncEnumerable<StructuredResult> LoadDataStructuredAsync(FileInfo fileInfo, ResultType? resultType = null, Dictionary<string, object>? metadata = null, Languages? language = null, CancellationToken cancellationToken = default)
    {
        if (!fileInfo.Exists)
        {
            throw new FileNotFoundException($"File {fileInfo.FullName} not found");
        ...
        var inMemoryFile =
            new InMemoryFile(File.ReadAllBytes(fileInfo.FullName), fileInfo.Name, FileTypes.GetMimeType(fileInfo.Name));
        return LoadDataStructuredAsync(inMemoryFile, resultType, metadata, language, cancellationToken);
    }
    ...
    public IAsyncEnumerable<StructuredResult> LoadDataStructuredAsync(InMemoryFile inMemoryFile, ResultType? resultType = null, Dictionary<string, object>? metadata = null, Languages? language = null, CancellationToken cancellationToken = default)
    {
        return LoadDataStructuredAsync([inMemoryFile], resultType, metadata, language, cancellationToken);
    }

    public async IAsyncEnumerable<StructuredResult> LoadDataStructuredAsync(
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        {
            var result = await job.GetStructuredResult(resultType ?? Configuration.ResultType, cancellationToken);
            yield return result;
        }
    }
    /// <summary>
