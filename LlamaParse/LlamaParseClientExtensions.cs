...
public static class LlamaParseClientExtensions
{
    ...
    public static IAsyncEnumerable<Document> LoadDataAsync(this LlamaParseClient llamaParseClient, 
        FileInfo file, bool splitByPage = false, 
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        if (extractTables)
        {
            await foreach (var table in llamaParseClient.LoadTablesAsync(rawResult))
            {
                if (documentByPage.Count > 0)
                {
                    table.ParentNode =
