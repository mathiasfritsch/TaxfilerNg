...
{
    public async Task<byte[]> GetImageAsync(string jobId, string imageName, CancellationToken cancellationToken)
    {
    ...
    public async Task<StructuredResult> GetJobStructuredOutputAsync(string jobId, ResultType resultType,
        CancellationToken cancellationToken)
    {
        var getResultUri =
            new Uri(
        ...
        var rootElement = JsonDocument.Parse(resultString).RootElement;
        var pagesElement = rootElement.GetProperty(Constants.PagesKey);
        
        return new StructuredResult
        {
            ResultPagesStructured = pagesElement.EnumerateArray()
                .Select( page => page.GetProperty("structuredData"))
                .ToArray()
        };
    }
        ...
        Configuration configuration, Languages? language, CancellationToken cancellationToken)
    {
        ...
        {
             form.Add(new StringContent("true"), "structured_output");
        }
        if (!string.IsNullOrWhiteSpace(configuration.StructuredOutputJsonSchemaName))
            form.Add(new StringContent(configuration.StructuredOutputJsonSchemaName), "structured_output_json_schema_name");
